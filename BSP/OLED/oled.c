...
/*
 * SSD1306 I2C transport. The controller FIFO is eight bytes deep, so every
 * transaction is one control byte plus no more than seven payload bytes.
 * A finite timeout prevents a missing display from hanging the whole car.
 */
static int OLED_I2C_Write(const u8 *data, u8 length)
{
...
void OLED_RefreshPage(u8 page)
{
	...
	packet[2]=0x00;
	packet[3]=0x10;
	if (!OLED_I2C_Write(packet,4))
		return;

	for(column=0;column<128;)
	{
		packet[0]=0x40;
		count=0;
		while((count<7) && (column<128))
		{
			packet[count+1]=OLED_GRAM[column][page];
			count++;
			column++;
		}
		if (!OLED_I2C_Write(packet,count+1))
			return;
	}
}
